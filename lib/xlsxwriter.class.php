...
function exportar_XLSX_bbdd($user,$pass,$bbdd,$host,$query,$rutaxls){
	$writer = new XLSXWriter();
	$writer->setAuthor('Proceso automatico PHP');


 	$omy = new mysqli($host, $user, $pass, $bbdd);
 	mysqli_set_charset($omy,"utf8");
 	$cabeceras_impresas=false;
    $resultado = $omy->query($query);
        if (!$resultado)
            return null;

        while ($fila = $resultado->fetch_assoc()) 
        {
       		if ($cabeceras_impresas==false){

				foreach($fila as $l=>$celda)
				{
					$filacabeza[]=$l;
				}
				 $writer->writeSheetRow('Sheet1',$filacabeza);

				$cabeceras_impresas=true;
				
			}
          $writer->writeSheetRow('Sheet1',$fila);

          }


	$writer->writeToFile($rutaxls);
	return $rutaxls;
}


function exportar_XLSX_bbdd_multiple($user,$pass,$bbdd,$host,$queries,$nombrehojas,$rutaxls){
	$writer = new XLSXWriter();
	$writer->setAuthor('Proceso automatico PHP');
 	...
 	foreach($queries as $query){
        ...
        if (!$resultado){
        	...
        	$nombrehoja=$nombrehojas[$numpaginas];
	        while ($fila = $resultado->fetch_assoc()) 
	        {
	       		if ($cabeceras_impresas==false){
					...
					foreach($fila as $l=>$celda)
					{
					 ...
					 $writer->writeSheetRow($nombrehoja,$filacabeza);

					$cabeceras_impresas=true;
					
				}
	          $writer->writeSheetRow($nombrehoja,$fila);
